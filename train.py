...
def train(rank, args, shared_model, optimizer, env_conf):
    ...
    while True:
        ...
        if player.done:
            if gpu_id >= 0:
                with torch.cuda.device(gpu_id):
                    player.cx = Variable(torch.zeros(1, lstm_size).cuda())
                    player.hx = Variable(torch.zeros(1, lstm_size).cuda())
            else:
                player.cx = Variable(torch.zeros(1, lstm_size))
                player.hx = Variable(torch.zeros(1, lstm_size))
        else:
