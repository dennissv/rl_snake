...
saved_state = torch.load(
    '{0}{1}_best.dat'.format(args.load_model_dir, args.env),
    ...
    while True:
        ...
        if args.render:
            if (i_episode % args.render_freq == 0):
                player.env.render(probs=player.log_probs[-1])
        ...
        if player.done and player.info['ale.lives'] > 0 and not player.max_length:
            ...
            log['{}_mon_log'.format(args.env)].info(
                "reward sum: {0}, reward mean: {1:.4f}, episode length: {2}".format(
                    reward_sum, reward_mean, player.eps_len))
            player.eps_len = 0
            break
